...
import com.arkonq.pixelweather.http.OpenWeatherApi
import com.arkonq.pixelweather.http.OpenWeatherClient
import com.arkonq.pixelweather.model.*
...
constructor(private val client: OpenWeatherClient) {
    ...
    private fun requestWeatherReport(): Observable<WeatherReport>? {
        ...
        when (weatherReportObservable) {
            null -> city?.let {
                weatherReportObservable = client.getCurrentWeather(it, CELSIUS, OpenWeatherApi.apiKey)
    ...
    private fun generateWeatherReport(currentWeather: CurrentWeatherModel): Single<WeatherReport> {
        return client.getForecast(city!!, CELSIUS, OpenWeatherApi.apiKey)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
